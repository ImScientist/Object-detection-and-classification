...
class CloudsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        masks_and_bboxes = get_masks_bboxes_of_disconnected_regions_from_encoded_pixels(data, img_height, img_width)

        masks = np.stack([el['mask'] for k, v in masks_and_bboxes.items() for el in v], axis=0)  # shape = (#masks, h,w)
        boxes = np.array([el['bbox'] for k, v in masks_and_bboxes.items() for el in v])
        labels = np.array([self.classmap[k] for k, v in masks_and_bboxes.items() for el in v])
